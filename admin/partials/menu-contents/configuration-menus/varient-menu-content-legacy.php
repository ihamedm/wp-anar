      ...
      <div class="property_select_container">
        ...
        <?php if ($awca_attributes) : ?>
          ...
          <?php foreach ($awca_attributes->items as $index => $item) : ?>
            <?php foreach ($item->attributes as $attrindex => $attritem) : ?>
              <?php
              $attribute = json_decode(json_encode($attritem), true);
              $attribute_id = isset($attritem->key) ? $attritem->key : '';
              $attribute_name = isset($attribute['name']) ? $attribute['name'] : 'Unknown';
              ?>

              <div class="varient_box">
                <?php
                    $match_cat = awca_find_best_match_category_from_categories($item->categories);
                  $name = awca_get_final_attribute_name_from_attribute_and_category($attribute_name, $match_cat);
                  $slug = awca_slugify($name);

                ?>
                <input type="hidden" name="anar-atts[<?php echo esc_html($slug); ?>]" value="<?php echo esc_html($slug); ?>" />
                <label>

                  <input type="text" name="anar-atts[<?php echo esc_html($slug); ?>]" disabled value="<?php echo esc_html($name); ?>" />
                </label>
                <?php

                if (!empty($woocommerce_attributes)) { ?>
                  <div class="select_wrapper">
                    <select name="product_attributes[<?php echo esc_html($slug); ?>]" class="varient_selcet">
                      <option value="select">انتخاب کنید</option>
                      <?php foreach ($woocommerce_attributes as $attribute) : ?>
                        <?php
                            $wc_name = awca_get_final_attribute_name_from_attribute_and_category($attribute->attribute_label, $match_cat);
                        // if we have existence an attribute before, the slug is saved in $attributeMap, we return selected on this check
                          ...
                          if (isset($attributeMap[$slug]) && $attributeMap[$slug] == awca_slugify($attribute->attribute_label)) {
                            $is_this_the_default_value = true;
                          }


                          // if is anar created attribute we save 'select' str as value in attributeMap, also we must convert the simple
                         // name saved in attributes to string that include the best category math for comparison
                          if (isset($attributeMap[$slug]) && ($attributeMap[$slug] == 'select' || $attributeMap[$slug] == null)
                                  && $slug == awca_slugify(awca_get_final_attribute_name_from_attribute_and_category($attribute->attribute_label, $match_cat))
                          ) {
                            $is_this_the_default_value = true;
                          }
                        ?>
                        <option value="<?php echo awca_slugify($attribute->attribute_label); ?>" <?php echo $is_this_the_default_value ? "selected='true'" : ''?> >
                            <?php
                    ...
                    <div class="select_icons_warpper">
                      <svg onclick="clear_select(<?php echo $count; ?>)" class="clear-icon" style="cursor: pointer; display:none;" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
