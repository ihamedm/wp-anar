<?php
$api_url = 'https://api.anar360.com/api/360/categories';
//$awca_categories = awca_get_data_from_api($api_url);
$awca_response = awca_get_stored_response('categories');
?>
...
<div class="stepContent" id="stepContent3">
    ...
    <?php  if(!isset($awca_response['response'])):
    ...
    <?php else:
        ...
        <div class="get-categories-alert <?php echo $categories_alert_classes;?>">
            ...
            <p>تاریخ آخرین بروزرسانی دسته بندی ها:
            <?php
            $formatted_date = mysql2date('j F Y' . ' ساعت ' . 'H:i', $awca_response['created_at']);
    ...
    <div class="categories_mapping_container">
        ...
        <form action="<?php echo admin_url('admin-ajax.php'); ?>" method="post" id="plugin_category_creation_form">
            <?php
                $awca_categories = $awca_response['response'];
            ...
            <?php if ($awca_categories != null) : ?>
                        ...
                        if (!empty($product_categories) && !is_wp_error($product_categories)) { ?>
                            <div style="position: relative; min-width:400px">
                                ...
                                <div class="select_icons_warpper" style="top:20px !important">
                                    <svg onclick="clear_select(<?php echo $index; ?> , 'category')" class="clear-cat-icon" style="cursor: pointer; display:none;" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="stepper_button_container">
                    <a onclick="awca_move_to_step(1)" class="plugin_activation_button prev_stepper_btn" name="category_create_submit" id="">
                        مرحله قبل
                    </a>
