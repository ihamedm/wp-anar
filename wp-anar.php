...
class Wp_Anar
{
    ...
    public function check_and_update_cron() {
        ...
        if ($installed_cron_version !== ANAR_CRON_VERSION) {
            $cron_jobs = new Core\CronJobs();
            $cron_jobs->reschedule_events();

            awca_log('Installed Cron Version: ' . $installed_cron_version);
            awca_log('New Cron Version: ' . ANAR_CRON_VERSION);
            awca_log('Cron job events rescheduled.');
