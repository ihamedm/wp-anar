...
class Gallery{
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_anar_dl_the_product_images_ajax', [$this, 'download_the_product_gallery_and_thumbnail_images_ajax']);
        ...
        add_action('wp_ajax_anar_dl_products_thumbnail_ajax', array($this, 'download_products_thumbnail_ajax'));
    ...
    public function add_dl_image_buttons_product_meta_box(){
        ...
        <a href="#" class="anar-ajax-action awca-btn awca-alt-btn awca-outline-btn" 
           ...
           data-action="anar_dl_the_product_images_ajax"
    ...
    public function download_products_thumbnail_ajax() {
        global $wpdb;
        // Verify nonce
        if (!isset($_POST['security_nonce']) || !wp_verify_nonce($_POST['security_nonce'], 'anar_dl_products_thumbnail_ajax_nonce')) {
            $this->log('Security nonce verification failed in download_products_thumbnail_ajax', 'error');
            wp_send_json_error(array(
                'message' => 'فرم نامعتبر است.'
            ));
        }
        ...
        // Check user capabilities
        if (!current_user_can('manage_woocommerce')) {
            $this->log('User lacks manage_woocommerce capability in download_products_thumbnail_ajax', 'error');
            wp_send_json_error(array(
                'message' => 'شما مجوز این کار را ندارید!'
            ));
        }
        ...
        $found_products = 0;
        $this_loop_products = 0;
        $queue_transient_key = 'anar_thumbnail_queue';
        $queue_ttl = 60 * 60; // 1 hour
        $product_ids = [];
        ...
        try {
            ...
            if ($page === 1) {
                ...
                $query = $wpdb->prepare("
                    SELECT p.ID FROM {$wpdb->posts} p
                    LEFT JOIN {$wpdb->postmeta} pm ON p.ID = pm.post_id AND pm.meta_key = '_anar_thumbnail_processed'
                    WHERE p.post_type = 'product'
                      AND p.post_status IN ('publish', 'draft')
                ...
                // Filter products that don't have thumbnails
                $filtered_ids = [];
                foreach ($all_ids as $product_id) {
                    ...
                    $has_thumbnail = $product->get_image_id();
                    if ($has_thumbnail) {
                        // Mark as processed since it already has thumbnail
                        update_post_meta($product_id, '_anar_thumbnail_processed', current_time('mysql'));
                        continue;
                    }
                    
                    // Check if product has image URL meta or can potentially fetch from API
                    $image_url = get_post_meta($product_id, '_product_image_url', true);
                    if (!empty($image_url) || true) { // Include all products without thumbnails, will try API if needed
                        $filtered_ids[] = $product_id;
                    }
                }
                
                set_transient($queue_transient_key, $filtered_ids, $queue_ttl);
            }
            ...
            if (!is_array($queue) || empty($queue)) {
                $has_more = false;
                $this->log('No more products found for thumbnail download (queue empty)', 'info');
                delete_transient($queue_transient_key);
                wp_send_json_success([
                    'success' => true,
                    'has_more' => false,
                    ...
                    'loop_products' => $this_loop_products,
                    'found_products' => 0,
                    'message' => 'تصاویر شاخص همه محصولات دانلود شده است',
                ]);
            }
            ...
            foreach ($product_ids as $product_id) {
                try {
                    $product = wc_get_product($product_id);
                    if (!$product) {
                        // Mark as processed even if product doesn't exist
                        update_post_meta($product_id, '_anar_thumbnail_processed', current_time('mysql'));
                        $total_processed++;
                        continue;
                    }
                    
                    // Double-check if product already has thumbnail (might have been set since queue was built)
                    $has_thumbnail = $product->get_image_id();
                    if ($has_thumbnail) {
                        update_post_meta($product_id, '_anar_thumbnail_processed', current_time('mysql'));
                        $total_processed++;
                    ...
                    // Get image URL from meta or fetch from API
                    $image_url = get_post_meta($product_id, '_product_image_url', true);
                    ...
                    if (empty($image_url)) {
                        ...
                        if (!$fresh_data['success']) {
                            $this->log(sprintf(
                                'Failed to fetch product data for product #%d: %s',
                                $product_id,
                                $fresh_data['message']
                            ), 'error');
                        ...
                        if (isset($fresh_data['data']->mainImage) && !empty($fresh_data['data']->mainImage)) {
                            ...
                            $this->log(sprintf(
                                'No mainImage found for product #%d',
                                $product_id
                            ), 'error');
                            // Mark as processed since no image available
                            update_post_meta($product_id, '_anar_thumbnail_processed', current_time('mysql'));
                            $total_processed++;
                            continue;
                        }
                    ...
                    $result = $this->image_downloader->set_product_thumbnail($product_id, $image_url);
                    if (!is_wp_error($result)) {
                        $total_downloaded++;
                    } else {
                        $this->log(sprintf(
                            'Error processing thumbnail for product #%d: %s',
                            $product_id,
                            $result->get_error_message()
                        ), 'error');
                    }
            ...
            $this->log('Exception in download_products_thumbnail_ajax: ' . $exception->getMessage(), 'error');
            $response = [
                'success' => false,
                'has_more' => false,
                'total_processed' => $total_processed,
